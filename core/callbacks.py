...
class BestModelTagger(Callback):
    ...
    def on_validation_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule):
        ...
        if monitor_val is None:
            raise RuntimeError(
                f"{self.monitor} was supposed to be logged from model. Could not find that"
            )
        if monitor_val is not None:
            if isinstance(monitor_val, Metric):
                ...
                monitor_val = torch.tensor(
                    monitor_val, device=pl_module.device, dtype=torch.float
                )
...
class MetricsLogger(Callback):
        ...
        if stages == "all":
            ...
            for stage in stages:
                if stage not in valid_stages:
                    raise ValueError(
                        f"Stage {stage} is not valid. Must be one of {valid_stages}"
                    )
    ...
    def _plot_metrics(trainer: pl.Trainer, pl_module: MalwareDetector, stage: str):
        confusion_matrix = (
            pl_module.test_outputs["confusion_matrix"].compute().cpu().numpy()
        )
        ...
        trainer.logger.experiment.log(
            {f"{stage}_confusion_matrix": wandb.Image(plt)}, commit=False
        )
        if stage != "test":
            return
        roc = pl_module.test_outputs["roc"].compute()
        figure = plot_curve(roc[0].cpu(), roc[1].cpu(), "roc")
        trainer.logger.experiment.log({f"ROC": figure}, commit=False)
        prc = pl_module.test_outputs["prc"].compute()
        figure = plot_curve(prc[1].cpu(), prc[0].cpu(), "prc")
        trainer.logger.experiment.log({f"PRC": figure}, commit=False)
    ...
    def on_train_epoch_end(
        self, trainer: pl.Trainer, pl_module: MalwareDetector, outputs
    ):
        if "train" not in self.stages:
            return
        trainer.logger.experiment.log(
            self.compute_metrics(pl_module, "train"), commit=False
        )
    ...
    def on_validation_end(self, trainer: pl.Trainer, pl_module: MalwareDetector):
        if "val" not in self.stages or trainer.running_sanity_check:
            return
        trainer.logger.experiment.log(
            self.compute_metrics(pl_module, "val"), commit=False
        )
    ...
    def on_test_end(self, trainer: pl.Trainer, pl_module: MalwareDetector):
        if "test" not in self.stages:
            return
        trainer.logger.experiment.log(
            self.compute_metrics(pl_module, "test"), commit=False
        )
