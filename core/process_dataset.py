...
class FeatureExtractors:
    ...
    def _get_instruction_type(op_value: int) -> str:
        ...
        if 0x00 == op_value:
            ...
            (0x3E <= op_value <= 0x43)
            or (op_value == 0x73)
            or (0x79 <= op_value <= 0x7A)
            or (0xE3 <= op_value <= 0xED)
        ):
            return "unused"
    ...
    def _get_api_trie() -> StringTrie:
        apis = open(
            Path(package_directory).parent / "metadata" / "api.list"
        ).readlines()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--source-dir", help="The directory containing apks", required=True
    )
    parser.add_argument(
        "-d",
        "--dest-dir",
