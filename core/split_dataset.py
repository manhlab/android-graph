...
def get_dataset(df: pd.DataFrame, test_ratio: float, log_dir: Path):
    ...
    combined = np.concatenate(
        [ben_hist[:, np.newaxis], mal_hist[:, np.newaxis]], axis=1
    )
    np.savetxt(log_dir / "histogram.list", combined)
    ...
    for i, (ben_size, mal_size) in enumerate(final_sizes):
        ...
        malware_samples = df.query(
            f'label == "Malware" and {low} <= num_nodes < {high}'
        )
        assert (
            len(benign_samples) >= ben_size and len(malware_samples) >= mal_size
        ), "Mismatch"
        benign_samples = benign_samples.sample(ben_size)
        malware_samples = malware_samples.sample(mal_size)
        if test_ratio > 0:
            benign_samples, benign_test_samples = np.split(
                benign_samples, [round((1 - test_ratio) * len(benign_samples))]
            )
            malware_samples, malware_test_samples = np.split(
                malware_samples, [round((1 - test_ratio) * len(malware_samples))]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input-dirs", help="List of input paths", nargs="+", required=True
    )
    parser.add_argument(
        "-o",
        "--output-dir",
    ...
    if len(zero_nodes) > 0:
        print(
            f"Warning: {len(zero_nodes)} APKs with num_nodes = 0 found. Writing their names to zero_nodes.list"
        )
